...
import src.index.node.TreeNode;
...
public class BinarySearchTree implements IndexInterface<TreeNode> {
    ...
    private TreeNode searchItem(TreeNode tNode, Comparable searchKey) {
        ...
        else if (tNode.key.compareTo(searchKey) == 0) return tNode;
        else if (tNode.key.compareTo(searchKey) > 0) return searchItem(tNode.left, searchKey);
    ...
    private TreeNode insertItem(TreeNode tNode, Comparable newItem) {
        if (tNode == null) tNode = new TreeNode(newItem, null, null);
        else if (tNode.key.compareTo(newItem) > 0) tNode.left = insertItem(tNode.left, newItem);
